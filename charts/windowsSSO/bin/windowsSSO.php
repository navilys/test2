...
/**
 * windowsSSO.php
 * This is the cron service that the plugin attaches to the main cron in order
  ...
  if (is_dir(PATH_DB . $sObject)) {
    ...
    if (file_exists(PATH_DB . $sObject . PATH_SEP . 'db.php')) {
      ...
      try {
        ...
        echo  $e->getMessage();
        eprintln('Problem in workspace: ' . $sObject.' it was ommited.', 'red');
      }
      eprintln();
    }
  }
...
if (!class_exists('windowsSSO')) {
  require_once PATH_RBAC . 'plugins/class.windowsSSO.php';
}

/**
 * Class used for the PM cron
 *
 * @author Julio Cesar Laura <juliocesar at colosa dot com> <contact at julio-laura dot com>
 * @package plugins.windowsSSO.classes
 * @copyright Copyright (C) 2004 - 2011 Colosa Inc.
 */
class windowsSSOClassCron {
   ...
   * this function will synchronize users from active directory to PM users tables
   * @return void
   */
  public function executeCron() {
    ...
    $plugin = new windowsSSO();

    // Get all authsource for this plugin ( windowsSSO plugin, because other authsources are not needed )
    ...
    foreach ($aAuthSources as $authSource) {
      ...
      // Process every department in the resulting array
      foreach ($aRegisteredDepts as $registeredDept) {
        $this->synchronizeAllDepartmentUsers($registeredDept, $plugin, $authSource);
      }

      foreach ($aRegisteredGroups as $registeredGroup) {
        $this->synchronizeAllGroupUsers($registeredGroup, $plugin, $authSource);
      }
  ...
  /**
   * Get the diff from two multi-dimensional arrays
   *
   * @param Array $array1 The first array
   * @param Array $array2 The second array
   * @return Array $difference The diff array
   */
  ...
  /**
   * Synchronize all users in the departments with a DN in PM
   *
   * @param Array $department The department information
   * @param Object $oPlugin A instance of the windowsSSO plugin
   * @param Array $authSource The authentication source information
   * @return Array $aUsers The users affected (new, modified)
   */
  private function synchronizeAllDepartmentUsers ($department, $oPlugin, $authSource) {
    $aUsers           = array();
    ...
    foreach($aLdapUsers as $ldapUser) {
      ...
      if (!$found) {
      	...
      	$newGroupCNArray = explode(',',$aUser['sDN']);
  ...
  /**
   * Synchronize all users in the groups with a DN in PM
   *
   * @param Array $group The department information
   * @param Object $oPlugin A instance of the windowsSSO plugin
   * @param Array $authSource The authentication source information
   * @return Array $aUsers The users affected (new, modified)
   */
  private function synchronizeAllGroupUsers($group, $oPlugin, $authSource) {
    $aUsers           = array();
    $unRegisteredUser = array();
