...
if (class_exists("enterprisePlugin")) {
  ...
  {  function pmReportsPlugin($sNamespace, $sFilename = null)
     {  $pathPluginTrunk = PATH_PLUGINS . "pmReports";
        ...
        $this->table    = array("PM_REPORT_PERMISSIONS");
    ...
    function enable()
    {
        ...
        while ($dataset->next()) {
            $reportPermissions[] = $dataset->getRow();
        }

        $criteria = new Criteria('workflow');
        $criteria->addSelectColumn(AdditionalTablesPeer::ADD_TAB_UID);
        $criteria->add(AdditionalTablesPeer::ADD_TAB_TAG, 'plugin@simplereport', Criteria::EQUAL);
        ...
        while ($dataset->next()) {
            ...
            foreach($reportPermissions as $key => $value) {
                if ($row['ADD_TAB_UID'] == $value['ADD_TAB_UID']) {
                    $sw = true;
                    continue;
                }
            }
            if (!$sw) {
                ...
                try {
                    $data['PMR_UID'] = $pmReportPermissionsInstance->createOrUpdate($data);
                }
