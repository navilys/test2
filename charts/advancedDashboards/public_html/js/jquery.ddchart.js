...
// Have fun!
	...
	jQuery.fn.ddBarChart = function(options) {
			...
			function resizeFonts (strSelector,blnTruncate) {
				intLabelPecent = 100;
				//$(strSelector).css('font-size',intLabelPecent+'%'); //***** Commented
		...
		function buildYAxis (objSettings, strID) {
			...
			//intYMax = intYMax + (intYStep - (intYMax % intYStep)); //***** Commented
   
   var strIntYMax = intYMax + "";                         //***** Added
   intYStep = Math.pow(10, strIntYMax.length) / 10;       //***** Added
   
   intYMax = intYMax + (intYStep - (intYMax % intYStep)); //***** Added
		...
		function buildChart (objSettings, strID, intYMax) {
			...
			intBarWidth = 100/objSettings.chartData.DATA.length; //***** Commented
   //intBarWidth = 150; //***** Added
    ...
    //***** Added
    strEvent = null;
    if (objSettings.chartData.COLUMNS.indexOf('EVENT_CLICK') >= 0) {
					strEvent = objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('EVENT_CLICK')];
				}
    
    //$(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub").append('<div class="'+objSettings.chartBarClass+' ddchart-chart" '+strToolTipTitle+strDrillLink+' barlabel="'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'" style="left:'+((intBarWidth*intBars)+(intMargin/2))+'%; height:'+((objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]/intYMax)*100)+'%; width:'+(intBarWidth-intMargin)+'%;'+strBGColor+'" chartParentID="'+strID+'">&nbsp;</div>'); //***** Commented
    $(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub").append('<div ' + ((strEvent != "")? 'onclick="'+strEvent+'"' : null) + ' class="'+objSettings.chartBarClass+' ddchart-chart" '+strToolTipTitle+strDrillLink+' barlabel="'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'" style="left:'+((intBarWidth*intBars)+(intMargin/2))+'%; height:'+((objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]/intYMax)*100)+'%; width:'+(intBarWidth-intMargin)+'%;'+strBGColor+'" chartParentID="'+strID+'">&nbsp;</div>'); //***** Added
    //$(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub").append('<div class="'+objSettings.chartBarClass+' ddchart-chart" '+strToolTipTitle+strDrillLink+' barlabel="'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'" style="left:'+((intBarWidth*intBars)+(intMargin/2))+'px; height:'+((objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]/intYMax)*100)+'%; width:'+(intBarWidth-intMargin)+'px;'+strBGColor+'" chartParentID="'+strID+'">&nbsp;</div>'); //***** Added
     ...
     if (!(typeof (objSettings.yLabel) === "undefined")) {
       for (var i = 0; i <= objSettings.yLabel.length - 1; i++) {
         yLabelAux = yLabelAux + objSettings.yLabel.charAt(i) + "<br />";
       }
     }
     
     //$(strID).html('<div class="ddchart-y-axis-wrapper"></div><div class="ddchart-x-axis-wrapper" ><div class="ddchart-x-axis-wrapper-sub" ></div><div class="'+objSettings.xWrapperClass+' ddchart-x-axis-label"><div style="display:inline" ></div></div></div><div class="ddchart-chart-wrapper ddchart-chart-final"></div>'); //***** Commented
     $(strID).html('<div class="ddchart-y-axis-label"><div>' + yLabelAux + '</div></div><div class="ddchart-y-axis-wrapper"></div>' + ((!(typeof (objSettings.zoom) === "undefined"))? '<div class="ddchart-zoom">' + objSettings.zoom + '</div>' : '') + '<div class="ddchart-x-axis-wrapper" ><div class="ddchart-x-axis-wrapper-sub" ></div><div class="'+objSettings.xWrapperClass+' ddchart-x-axis-label"><div style="display:inline" ></div></div></div><div class="ddchart-chart-wrapper ddchart-chart-final"></div>'); //***** Added
				}
			}
