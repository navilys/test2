...
abstract class BaseAddonsStore extends BaseObject  implements Persistent {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setStoreVersion($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setStoreLocation($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setStoreType($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setStoreLastUpdated($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AddonsStorePeer::STORE_ID)) $criteria->add(AddonsStorePeer::STORE_ID, $this->store_id);
		if ($this->isColumnModified(AddonsStorePeer::STORE_VERSION)) $criteria->add(AddonsStorePeer::STORE_VERSION, $this->store_version);
		if ($this->isColumnModified(AddonsStorePeer::STORE_LOCATION)) $criteria->add(AddonsStorePeer::STORE_LOCATION, $this->store_location);
		if ($this->isColumnModified(AddonsStorePeer::STORE_TYPE)) $criteria->add(AddonsStorePeer::STORE_TYPE, $this->store_type);
		if ($this->isColumnModified(AddonsStorePeer::STORE_LAST_UPDATED)) $criteria->add(AddonsStorePeer::STORE_LAST_UPDATED, $this->store_last_updated);
