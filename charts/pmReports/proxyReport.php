...
try {
  ...
  $oCriteria->addJoin($tableName . ".APP_UID", ApplicationPeer::APP_UID, Criteria::INNER_JOIN);
  ...
  if (is_object($filterList)) {
    ...
    foreach ($filterListData as $fieldName => $fieldValue) {
      $fieldType = strtoupper($types[$fieldName]["FLD_TYPE"]);

      if ((($fieldType == "DATE" || $fieldType == "TIMESTAMP") && ($filterList->{$fieldName . "__FROM__"} != "" || $filterList->{$fieldName . "__TO__"} != "")) ||
          ($fieldValue != "" && $filterList->{$fieldName . "_operator"} != "" && $filterList->{$fieldName . "_operator"} != "- EMPTY -")
         ) {
    ...
    foreach ($aRow as $dataKey => $dataField) {
      foreach($comboBoxList as $comboBoxField) {
        if ($comboBoxField == $dataKey) {
          ...
          if (isset($filterList->{$comboBoxField}) && !empty($filterList->{$comboBoxField}) && $filterList->{$comboBoxField . "_operator"} != "- EMPTY -") {
