...
function pmReportsPermissions($userUid) {
    try {
        //echo $userUid;
        ...
        $criteria->add(PmReportPermissionsPeer::PMR_TYPE, 'EVERYBODY');
        $criteria->add(PmReportPermissionsPeer::PMR_STATUS, '1');
        $dataset = PmReportPermissionsPeer::doSelectRS($criteria);
        $dataset->setFetchmode(ResultSet::FETCHMODE_ASSOC);
        $dataset->next();
        while ($row = $dataset->getRow()) {
           $reportTable .= "'".$row['ADD_TAB_UID']."',";
           $dataset->next();
        }
        ...
        $criteria->addSelectColumn(PmReportPermissionsPeer::PMR_UID);
        $criteria->addSelectColumn(PmReportPermissionsPeer::ADD_TAB_UID);
        $criteria->add(PmReportPermissionsPeer::PMR_TYPE, 'USER');
        $criteria->add(PmReportPermissionsPeer::PMR_OWNER_UID, $userUid);
        $criteria->add(PmReportPermissionsPeer::PMR_STATUS, '1');
        $dataset = PmReportPermissionsPeer::doSelectRS($criteria);
        $dataset->setFetchmode(ResultSet::FETCHMODE_ASSOC);
        $dataset->next();
        ...
        foreach ($departments as $depUid => $department) {
            ...
            $criteria->addSelectColumn(PmReportPermissionsPeer::PMR_UID);
            $criteria->addSelectColumn(PmReportPermissionsPeer::ADD_TAB_UID);
            $criteria->add(PmReportPermissionsPeer::PMR_TYPE, 'DEPARTMENT');
            $criteria->add(PmReportPermissionsPeer::PMR_OWNER_UID, $depUid);
            $criteria->add(PmReportPermissionsPeer::PMR_STATUS, '1');
            $dataset = PmReportPermissionsPeer::doSelectRS($criteria);
            $dataset->setFetchmode(ResultSet::FETCHMODE_ASSOC);
            $dataset->next();
        ...
        foreach ($groups as $grpUid => $group) {
            ...
            $criteria->addSelectColumn(PmReportPermissionsPeer::PMR_UID);
            $criteria->addSelectColumn(PmReportPermissionsPeer::ADD_TAB_UID);
            $criteria->add(PmReportPermissionsPeer::PMR_TYPE, 'GROUP');
            $criteria->add(PmReportPermissionsPeer::PMR_OWNER_UID, $grpUid);
            $criteria->add(PmReportPermissionsPeer::PMR_STATUS, '1');
            $dataset = PmReportPermissionsPeer::doSelectRS($criteria);
            $dataset->setFetchmode(ResultSet::FETCHMODE_ASSOC);
            $dataset->next();
        ...
        $reportTable = substr ( $reportTable , 0, -1 );
        $reportTable .=")";
        return $reportTable;
    }
...
$listReport = pmReportsPermissions($userUid);

$sql = "SELECT ADD_TAB_NAME, ADD_TAB_DESCRIPTION, ADD_TAB_UID, PRO_UID
        FROM   ADDITIONAL_TABLES
        WHERE  ADD_TAB_TAG = 'plugin@simplereport' and ADD_TAB_UID IN ".$listReport;
$resultSQL = executeQuery($sql);


$i = 0;
foreach ($resultSQL as $key => $value) {
  $tasTitle = trim($resultSQL[$key]['ADD_TAB_DESCRIPTION']) != '' ? $resultSQL[$key]['ADD_TAB_DESCRIPTION'] : substr($resultSQL[$key]['ADD_TAB_NAME'], 4) ;
...
if (class_exists('pmSLAPlugin')) {
    $aData[] = "  {title: 'SLA Report',
                   layout: 'fit',
                   items: [ new Ext.ux.IFrameComponent({ id: 'id_tab_pmsla', url: '../pmSLA/slaReport' }) ]
                  }";
}
...
$oHeadPublisher->assign("PATH_PLUGINS", PATH_PLUGINS);
