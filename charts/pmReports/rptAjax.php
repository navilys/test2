...
class ajax_rpt extends WebResource
{
	function save_pm_report($sRepTabUid, $sPmrUid, $sProUid, $sPmrTitle, $sPmrDescription, $sPmrStatus, $sDynUid){
		
		if( $sPmrUid == '' )
			$sPmrUid = G::generateUniqueID();
			
		G::LoadClass('reportTables');
		if($sPmrStatus == '1' && $sDynUid!=''){
			
			//create report table
			$sTableName = 'PMR_'.$sPmrUid;
			$_POST['form']['REP_TAB_UID'] = $sRepTabUid;
			$_POST['form']['PRO_UID'] = $sProUid;
			$_POST['form']['REP_TAB_NAME'] = $sTableName;
			$_POST['form']['REP_TAB_TYPE'] = 'GRID';
			$_POST['form']['REP_TAB_GRID'] = $_POST['form']['PRO_UID'].'-'.$sDynUid;
			$_POST['form']['REP_TAB_CONNECTION'] = 'wf';
			$_POST['form']['REP_TAB_CREATE_DATE'] = date("Y-m-d H:i:s");
			$_POST['form']['REP_TAB_STATUS'] = 'ACTIVE';
			$_POST['form']['REP_TAB_TITLE'] = 'PMR_'.$sPmrUid;
		...
		if(!(is_object($oPmReport)) || get_class($oPmReport)!='PmReport'){
			$oPmReport = new PmReport();
			$oPmReport->setPmrUid($sPmrUid);
		}
		if($sPmrStatus == '1'){
			$oPmReport->setPmrStatus('ACTIVE');
		}
		else{
			$oPmReport->setPmrStatus('INACTIVE');
		}
		if($sPmrStatus == '2'){
			$oPmReport->setPmrStatus('DELETED');
		}
		
		$oPmReport->setDynUid($sDynUid);
		$oPmReport->setRepTabUid($sRepTabUid);
		$oPmReport->save();
		//Content
		require_once ('classes/model/Content.php');
		$oContent = new Content();
		$oContent->addContent('PMR_TITLE', '', $sPmrUid, SYS_LANG, $sPmrTitle);
		$oContent->addContent('PMR_DESCRIPTION', '', $sPmrUid, SYS_LANG, $sPmrDescription);
		
		$sClassName = 'PMR_'.$sPmrUid;
		if($sPmrStatus == '1'){
			$oAdditionalTables = new AdditionalTables();//AdditionalTablesConsolidated
			
			$oAdditionalTables->createPropelClasses($sTableName, $sClassName, $aFieldsClases, $sPmrUid);
		}
		else{
			$sPath = PATH_DB . SYS_SYS . PATH_SEP . 'classes' . PATH_SEP;
	...
	function delete_pm_report($sPmrUid){
		require_once ( "classes/model/PmReportPeer.php" );
		require_once ( "classes/model/PmReport.php" );
		require_once ( 'classes/model/Content.php');
		$oPmReport = PmReportPeer::retrieveByPK($sPmrUid);
		if(is_object($oPmReport) && get_class($oPmReport) == 'PmReport'){
			$sRepTabUid = $oPmReport->getRepTabUid();
			$sPmrUid = $oPmReport->getPmrUid();
			$sPmrStatus = 2;//this status will set the PM Report to deleted
			$sDynUid = $oPmReport->getDynUid();
			$oContent = new Content();
			$sPmrTitle = $oContent->load('PMR_TITLE', '', $oPmReport->getPmrUid(), SYS_LANG);
			$sPmrDescription = $oContent->load('PMR_DESCRIPTION', '', $oPmReport->getPmrUid(), SYS_LANG);
			$sProUid = $_SESSION['PROCESS'];
			$this->save_pm_report($sRepTabUid, $sPmrUid, $sProUid, $sPmrTitle, $sPmrDescription, $sPmrStatus, $sDynUid);
		}
	}
	function add_permission_to_pm_report($sPmrUid, $sUsrUid, $iPmrpType){
		require_once ( "classes/model/PmReportPermissions.php" );
		require_once ( "classes/model/PmReportPermissionsPeer.php" );
		$oPmReportPermission = PmReportPermissionsPeer::retrieveByPk($sPmrUid,$sUsrUid);
		if(!(is_object($oPmReportPermission)) || get_class($oPmReportPermission) != 'PmReportPermissions'){
			$oPmReportPermission = new PmReportPermissions();
			$oPmReportPermission->setPmrUid($sPmrUid);
			$oPmReportPermission->setUsrUid($sUsrUid);
		}
		$oPmReportPermission->setPmrpType($iPmrpType);
		$oPmReportPermission->save();
	}
	function remove_permission_to_pm_report($sPmrUid, $sUsrUid, $iPmrpType){
		require_once ( "classes/model/PmReportPermissions.php" );
		require_once ( "classes/model/PmReportPermissionsPeer.php" );
		$oPmReportPermission = PmReportPermissionsPeer::retrieveByPk($sPmrUid,$sUsrUid);
		if(is_object($oPmReportPermission) && get_class($oPmReportPermission) == 'PmReportPermissions'){
			PmReportPermissionsPeer::doDelete($oPmReportPermission);
		}
	}
}

$o = new ajax_rpt( $_SERVER['REQUEST_URI'], $_POST );
