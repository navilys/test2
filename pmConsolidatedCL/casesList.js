  ...
  if(rowModel){
    ...
    switch(action){
      ...
      default:
        ...
        requestFile = '../../green/cases/cases_Open';
        break;
    }
    params += '&action=' + 'todo';
...
function pauseCase(date){
  ...
  Ext.Msg.confirm(
    ...
    function(btn, text){
      if ( btn == 'yes' ) {
        Ext.MessageBox.show({
          msg: TRANSLATIONS.ID_PROCESSING,
          wait:true,
          waitConfig: {
            interval:200
          }
...
Ext.onReady(function () {
  Ext.Ajax.request({
    ...
    success: function(response) {
      ...
      //alert(dataResponse.conCount);
      parent.document.getElementById('conTotalCount').innerHTML = dataResponse.conCount;
    },
  ...
  {xtype: 'checkbox', //defining the type of component  
   fieldLabel: 'Apply changes to all rows',//assigning a label   
   name: 'chk_allColumn', //and a "name" so we can retrieve it in the server...  
   id: 'chk_allColumn'// ...when the form is sent  
  },
  textSearch,
  resetSearchButton,
  btnSearch,
               ...
               Ext.Msg.show({
                 ...
                 fn: function (btn) {
                   if (btn == "yes") {
                     ...
                     var dataUpdate = "";
                     var strValue = strReplace("\"", "\\\"", e.value + "");
                     var sw = 0;
