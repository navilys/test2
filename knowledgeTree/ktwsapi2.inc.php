...
//G::pr($ch);
    $response = curl_exec($ch);
    //print "<textarea rows='20' cols='50'>$response</textarea>";
    curl_close($ch);
...
class KTWSAPI_Folder extends KTWSAPI_FolderItem {
   ...
   * @return KTWSAPI_Folder
   */
  function &get_folder_by_name($foldername, $parent=1) {
    ...
    //print "Full path get_folder_by_name: $path<br />";
    $kt_folder_detail = $this->ktapi->soapclient->get_folder_detail_by_name($this->ktapi->session, $path);

    if (SOAP_Client::isError($kt_folder_detail)) {
      //Try another version of this method!!
      $kt_folder_detail = $this->ktapi->soapclient->get_folder_detail_by_name($this->ktapi->session, $foldername, $parent);
      if (SOAP_Client::isError($kt_folder_detail)) {
        return $kt_folder_detail;
      }
    }
