...
if (isset($_SESSION['G_MESSAGE_TYPE']) && isset($_SESSION['G_MESSAGE'])) {
 ...
 echo $folderContent;exit;
  ...
  $htmlGroup   = <<<GHTML
	...
	<td width='' class='treeNode' style='border:0px;background-color:transparent;'><a href="#" onclick="kt_toggleFolder('{$obj->id}');return false;"><img src="/plugin/knowledgeTree/images/folderV2.gif" border = "0" valign="middle" />&nbsp;{$obj->title}</a>&nbsp;<small>{$obj->created_by}</small></td>
  ...
  $pluginObj = new knowledgeTreeClass ();
	...
	case "ktDmsMapFields":
      ...
      foreach($aFields as $key => $fieldObj){
        $aFields1 = array('VAR_NAME' => $fieldObj['sName'], 'VAR_LABEL' => $fieldObj['sName']." (".$fieldObj['sLabel'].")");
