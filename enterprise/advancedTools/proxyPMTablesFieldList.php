<?php
   ...
   */
   ...
   function setDefaultFields() {
     ...
     $fields[] = array( 'name' => 'USR_UID'                , 'fieldType' => 'case field' , 'label' => G::loadTranslation('ID_CASESLIST_USR_UID')          ,      'width' => 100, 'align' => 'left');
     $fields[] = array( 'name' => 'PREVIOUS_USR_UID'       , 'fieldType' => 'case field' , 'label' => G::loadTranslation('ID_CASESLIST_PREVIOUS_USR_UID') ,      'width' => 100, 'align' => 'left');
  ...
  */
  function xActionRead () {
    ...
    if ( ! $validConfig ) {
      ...
      print G::json_encode( $confCasesList ) ;
    }
  }
...
//    	if ( $val == 'APP_UID' || $val == 'DEL_INDEX' ) unset ( $first[$key] );
      if ( $val == 'APP_UID' || $val == 'DEL_INDEX' || $val == 'USR_UID' || $val == 'PREVIOUS_USR_UID' ) unset ( $first[$key] );
    }
    foreach ( $second as $key => $val ) {
    	if ( $val->name == 'APP_UID' || $val->name == 'DEL_INDEX' || $val->name == 'USR_UID' || $val->name == 'PREVIOUS_USR_UID') unset ( $second[$key] );
//    	if ( $val->name == 'APP_UID' || $val->name == 'DEL_INDEX' ) unset ( $second[$key] );
    ...
    $previousUsrUid->gridIndex = 0;
    $previousUsrUid->label = '';
    $previousUsrUid->align = '';
    $previousUsrUid->width = 1;
    ...
    foreach ( $second as $key => $val ) {
    	$fieldType  = 'PM Table';
    	$fieldName  = $val->name;
      $fieldLabel = isset($val->label) && strlen(trim($val->label)) > 0 ? $val->label : $fieldName;
