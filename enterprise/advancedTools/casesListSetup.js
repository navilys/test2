  ...
  function sendGridFieldsRequest(action) {
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        ...
        if ( secondGrid.store.data.items[3].data['name'] == 'PREVIOUS_USR_UID' ) secondGrid.store.removeAt(3); //app_uid
        if ( secondGrid.store.data.items[2].data['name'] == 'USR_UID'   ) secondGrid.store.removeAt(2); //app_uid
