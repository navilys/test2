...
{
    require_once (PATH_PLUGINS . "enterprise/bin/tasks/cliAddons.php");
...
try {
    ...
    if (isset($_REQUEST['addon']) && isset($_REQUEST['store'])) {
        require_once (PATH_PLUGINS . 'enterprise/classes/model/AddonsManagerPeer.php');
      ...
      case "importlicense":
        if (sizeof($_FILES) > 0) {
          ...
          if (trim($aExtentionFile[sizeof($aExtentionFile) - 1]) != "dat") {
            ...
            G::header("location: pluginsList"); //ok
            exit(0);
          }
        }
        break;
