...
Ext.onReady(function(){
  ...
  configUsers = new Ext.Action({
	  text:('Config Information Users'),
	  iconCls :'button_menu_ext ss_configUsers',
	  handler: ConfigUserPage,
	  disabled: false 
  });
  
  configListUsers = new Ext.Action({
 ...
 // columns estaticas  
  column = {id: 'USR_UID', dataIndex: 'USR_UID', hidden: true, hideable: false};
  _columns.push(column);
  column ={header: _('ID_USER_NAME'), dataIndex: 'USR_USERNAME', width: 90, align: 'left', sortable: true};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_FULL_NAME'), dataIndex: 'USR_USERNAME', width: 175, align: 'left', renderer: full_name};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_EMAIL'), dataIndex: 'USR_EMAIL', width: 120, hidden: true, align: 'left', sortable: true};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_STATUS'), dataIndex: 'USR_STATUS', width: 50, align: 'center', renderer: render_status, sortable: true};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_ROLE'), dataIndex: 'USR_ROLE', width: 150, align:'left', sortable: true};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_DEPARTMENT'), dataIndex: 'DEP_TITLE', width: 150, hidden: true, align: 'left'};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_LAST_LOGIN'), dataIndex: 'LAST_LOGIN', width: 108, align: 'center', renderer: render_lastlogin};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_AUTHENTICATION_SOURCE'), dataIndex: 'USR_AUTH_SOURCE', width: 108, hidden: true, align: 'left'};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column ={header: _('ID_CASES_NUM'), dataIndex: 'TOTAL_CASES', width: 75, align:'right', sortType: 'asInt'};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
  column = {header: _('ID_DUE_DATE'), dataIndex: 'USR_DUE_DATE', width: 108, align:'center', renderer: render_duedate, sortable: true};
  column.editor = {
	        xtype : 'displayfield',
	        style : 'font-size:11px; padding-left:7px'		      
	    }
  _columns.push(column);
 // end columns estaticas
  
 // fields estaticas 
  _fields.push({name: _idProperty});
  
  _fields.push({name : 'USR_UID'});
  _fields.push({name : 'USR_USERNAME'});
  _fields.push({name : 'USR_FIRSTNAME'});
  _fields.push({name : 'USR_LASTNAME'});
  _fields.push({name : 'USR_EMAIL'});
  _fields.push({name : 'USR_ROLE'});
  _fields.push({name : 'USR_DUE_DATE'});
  _fields.push({name : 'DEP_TITLE'});
  _fields.push({name : 'LAST_LOGIN'});
  _fields.push({name : 'USR_STATUS'});
  _fields.push({name : 'TOTAL_CASES',type:'int'});
  _fields.push({name : 'DUE_DATE_OK'});
  _fields.push({name : 'USR_AUTH_SOURCE'});
  ...
  infoGrid = new Ext.grid.GridPanel({
    ...
    cm: cmodel,
    sm: smodel,
    tbar: [newButton, '-',summaryButton,'-', editButton, changeStatusButton, deleteButton, '-',authenticationButton, configUsers, {xtype: 'tbfill'}, searchText,clearTextButton,searchButton],
    bbar: bbarpaging,
    listeners: {
...
//Open Config-User Administration
ConfigUserPage = function(value){
  rowSelected = infoGrid.getSelectionModel().getSelected();
  
  	location.href = 'configUsers?uUID=' + value + '&type=auth';;
