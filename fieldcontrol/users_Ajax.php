  ...
  switch ($value){
    ...
    case 'usersList':
      ...
      while($oDataset->next()){
        ...
        $queryP = "SELECT CFG_LIST_USR_FIELD_NAME
				  FROM PMT_CONFIG_LIST_USERS NU
				  ";
		$datListUser = executeQuery($queryP);
		foreach($datListUser as $list)
		{
				$query = "SELECT NU.USR_NEW_INF_FIELD_NAME, NU.USR_NEW_INF_VALUE_FIELD, CU.CFG_USR_PARAMETERS, CU.CFG_USR_TYPE_ACTION, CU.CFG_USR_TYPE, NU.CFG_USR_FIELD_NAME
				  		  FROM PMT_USER_NEW_INFORMATION NU
				  		  INNER JOIN PMT_CONFIG_USERS CU ON (CU.CFG_USR_FIELD_NAME = NU.CFG_USR_FIELD_NAME)
				  		  WHERE  USR_UID = '".$row['USR_UID']."' AND NU.USR_NEW_INF_FIELD_NAME = '".$list['CFG_LIST_USR_FIELD_NAME']."'
				 	     ";
				$datNewUser = executeQuery($query);
				if(sizeof($datNewUser))
				{
					foreach($datNewUser as $index)
					{				
						if($index['CFG_USR_TYPE'] == 'DROPDOWN')
						{
							if($index['CFG_USR_TYPE_ACTION'] == 'QUERY' )
							{
								$queryConfig = $index['CFG_USR_PARAMETERS'];
								$aData =  executeQuery ($queryConfig);
								foreach($aData as $row1)
								{
									if($index['USR_NEW_INF_VALUE_FIELD'] == $row1['ID'] )
										$row[$index['USR_NEW_INF_FIELD_NAME']] = $row1['NAME'];
								}
							}
							else
							{
								$sQuery = " SELECT CFG_USR_OPT_ID_OPTION AS ID, CFG_USR_OPT_DESCRIPTION AS NAME
                      				...
                      				WHERE CFG_USR_FIELD_NAME = '".$index['CFG_USR_FIELD_NAME']."'
                       				ORDER BY ID";
							...
							$row[$index['USR_NEW_INF_FIELD_NAME']] = $index['USR_NEW_INF_VALUE_FIELD'];
					}
				}
