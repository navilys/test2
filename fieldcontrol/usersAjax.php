...
{  
  ...
  case 'saveUser' :
    try { 
	  ...
	  if ($form['USR_UID'] == '') {
        ...
        $role=$aData['USR_ROLE']   = $form['USR_ROLE'];
        ...
        $oConf->saveConfig('USER_PREFERENCES', '', '',$_SESSION['USER_LOGGED']);
      	...
      	$query = "SELECT GRP_UID FROM GROUPWF
      			 ...
      			 WHERE CON_VALUE = '".$role."' AND CON_CATEGORY = 'GRP_TITLE'
      			";
      	...
      	$delQuery = "DELETE FROM PMT_USER_NEW_INFORMATION WHERE USR_UID = '" . $sUserUID ."' ";
		$delete = executeQuery ($delQuery);
      	
      	$queryId = "SELECT max(USR_NEW_INF_ID) AS MAX_ID FROM  PMT_USER_NEW_INFORMATION  ";
		$maxId = executeQuery ( $queryId );
		$sgtIdIn = $maxId[1]['MAX_ID'] + 1;
	
		$queryOptions = "SELECT CFG_USR_FIELD_NAME AS FIELD_NAME, CFG_USR_TYPE AS TYPE  FROM PMT_CONFIG_USERS";
		$options = executeQuery($queryOptions);
		$arrayOptions = Array();
		foreach($options as $index)
		{
			if($index['TYPE'] == 'DROPDOWN')
			{
				$valueField = $index['id_'.$index['FIELD_NAME']] =  $form['id_'.$index['FIELD_NAME']];
				$fieldName = 'id_'.$index['FIELD_NAME'];
			}
			else 
			{
				$valueField = $index['name_'.$index['FIELD_NAME']] =  $form['name_'.$index['FIELD_NAME']];
				$fieldName = 'name_'.$index['FIELD_NAME'];
			}
				
			$insert = "INSERT INTO PMT_USER_NEW_INFORMATION (
						USR_NEW_INF_ID,
						USR_UID,
						USR_NEW_INF_FIELD_NAME,
						USR_NEW_INF_VALUE_FIELD,
						CFG_USR_FIELD_NAME
						)
						VALUES(
						'" . $sgtIdIn . "',
  ...
  case 'userData':
	...
	G::LoadClass('pmFunctions');
	$query = "SELECT * FROM PMT_USER_NEW_INFORMATION WHERE USR_UID = '" . $_POST['USR_UID'] . "' ";
	$dataNew = executeQuery($query);
	foreach($dataNew as $index)
	{
		$aFields[$index['USR_NEW_INF_FIELD_NAME']] = $index['USR_NEW_INF_VALUE_FIELD'];
	}
    $result->success = true;
    $result->user = $aFields;
    print(G::json_encode($result));
