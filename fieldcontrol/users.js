...
Ext.onReady(function () {
  ...
  var i;
	...
	{		
		if(arrayOptions[i].TYPE == 'DROPDOWN')
		{
			...
			combo = " combo_" + arrayOptions[i].FIELD_NAME + " = new Ext.form.ComboBox({fieldLabel : '" +  arrayOptions[i].DESCRIPTION +"' , id : 'id_" + arrayOptions[i].FIELD_NAME + "', store : store_" + arrayOptions[i].FIELD_NAME + " , valueField : 'ID', displayField  : 'NAME', triggerAction : 'all', width : 260, autocomplete  : true, typeAhead : true, mode : 'local', hiddenName : 'id_" + arrayOptions[i].FIELD_NAME + "'  }); ";
			//console.log(combo);
			eval(combo);
			if(j == 0)
			{
				a = " combo_" + arrayOptions[i].FIELD_NAME ;
				b = "eval (" + a + ")";
			}	
			else
			{
		...
		{
			textName = " text_" + arrayOptions[i].FIELD_NAME + " = new Ext.form.TextField({fieldLabel : '" +  arrayOptions[i].DESCRIPTION +"' , id : 'id_" + arrayOptions[i].FIELD_NAME + "', name: 'name_" + arrayOptions[i].FIELD_NAME + "', width : 260,  allowBlank: true  }); " ;
			eval(textName);
			
			if(j == 0)
			{
				a = " text_" + arrayOptions[i].FIELD_NAME ;
				b = "eval (" + a + ")";
			}	
			else
			{
  ...
  eval(" var newConfig = new Ext.form.FieldSet({title : 'New Config', items : [ "+ itemsOptions  + " ] });");
    ...
    Ext.Ajax.request({
        ...
        success: function (r, o) {
        	...
        	{		
        		if(arrayOptions[i].TYPE != 'DROPDOWN')
        		{
        			textData = " Ext.getCmp('frmDetails').getForm().setValues({ name_" + arrayOptions[i].FIELD_NAME + " : data.user.name_" + arrayOptions[i].FIELD_NAME + " });  ";
        			eval(textData);
        		}
        	...
        	{		
        		if(arrayOptions[i].TYPE == 'DROPDOWN')
        		{
        			var combo = " store_" + arrayOptions[i].FIELD_NAME + ".load(); combo_" + arrayOptions[i].FIELD_NAME + ".store.on('load', function(store) {" + " combo_" + arrayOptions[i].FIELD_NAME + ".setValue(data.user.id_" + arrayOptions[i].FIELD_NAME + ");  }); ";
        			eval(combo);        			
        		}
        	}
