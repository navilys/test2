...
class ldapAdvanced
{
  ...
  function VerifyLogin( $strUser, $strPass) {
    ...
    try {
      ...
      // check if the dn in the database record matches with the dn for the ldap
      // account
      $userDN           = ldap_explode_dn($strUser,1);
      $userDNArray      = $this->custom_ldap_explode_dn($strUser);
      $userDNIdentifier = explode('=',$userDNArray[0]);
      $verifiedUser = $this->searchUserByUid($userDN[0],$userDNIdentifier[0]);

      if ( $verifiedUser['sDN'] != $strUser && $verifiedUser['sDN']!=null ){
      ...
      $aAuthSource['AUTH_ANONYMOUS']          = '0';
      $aAuthSource['AUTH_SOURCE_SEARCH_USER'] = $verifiedUser['sDN'];
      $aAuthSource['AUTH_SOURCE_PASSWORD']    = $strPass;

      if ( trim($verifiedUser['sDN']) == '' ) return -1;
      ...
      if ($this->userIsTerminated($userDN[0],$attributes['AUTH_SOURCE_RETIRED_OU'])) {
        $this->deactivateUser($userDN[0]);
...
*/
    ...
    $sFilter .= "(|($uidUser=$sKeyword)$altFilter))";
    $aUser  = null;
    $oSearch = @ldap_search($oLink, $aAuthSource['AUTH_SOURCE_BASE_DN'], $sFilter,  array('dn','uid','samaccountname','cn','givenname','sn','mail','userprincipalname') );
    if ($oError = @ldap_errno($oLink)) {
      return $aUser;
    }
    else {
      if ($oSearch) {
        if (@ldap_count_entries($oLink, $oSearch) > 0) {
          ...
          do {
            ...
            $sUsername = isset($aAttr[ $uidUser ]) ? $aAttr[ $uidUser ] : '';
            if ($sUsername != '') {
              $aUser = array('sUsername'   => $sUsername,
  ...
  */
  ...
  /**
   * Get a deparment list based in a keyword search
   * @param String $sKeyword
   * @return <type>
   */
  function searchDepartments($sKeyword ) {
    $sKeyword     = trim($sKeyword);
 ...
 */
  ...
  */
  ...
  /**
   * Get a group list based in a keyword search
  ...
  function searchGroups($keyword) {
    ...
    //prefix
    $keyword = ( (substr($keyword, 0,1) != '*') ? '*' : '' ) . $keyword ;
    //sufix
    $keyword = $keyword . ( (substr($keyword, -1) != '*') ? '*' : '' );

    $sFilter = $filter = '(&(objectCategory=group)(name=*))';
