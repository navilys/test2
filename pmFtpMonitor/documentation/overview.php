...
/**
  ...
  The FTP Monitor Plugin is an extension that can be installed in a ProcessMaker server
  in order to bring ability to create Cases based on the files found on the FTP server.

  As soon as cron gets executed the names of the files on the FTP server get tried to match the
  pattern set up while the scheduler had been created. If the pattern is empty all the files get considered.
  The files matched get verified that they already haven't been uploaded to the server and exist in the log 
  table in the database. For each new file matched a new case gets created and the file gets uploaded to the 
  server as an Uploaded Document to the Case.  
  ...
  - @ref SEC_SETTINGS_LIST
  - @ref SEC_SETTINGS_FTP
  - @ref SEC_SETTINGS_SHARED
  - @ref SEC_SETTINGS_DISABLED
  - @ref SEC_LOG
  - @ref SEC_LOG_DETAILS
...
/**
  ...
  @section SEC_SETTINGS_LIST Setting List
