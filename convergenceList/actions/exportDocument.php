...
if (count($getVar) == 1) {
    ...
    $query = 'SELECT * FROM APP_DOCUMENT, CONTENT WHERE APP_UID="'.$getVar[0]['APP_UID'].'" AND DOC_UID="'.$_REQUEST['iddoc'].'" AND APP_DOC_TYPE="OUTPUT" AND APP_DOC_STATUS="ACTIVE" AND APP_DOC_UID = CON_ID AND CON_CATEGORY = "APP_DOC_FILENAME" AND CON_LANG = "'.SYS_LANG.'" ';
