...
abstract class BaseAbeRequests extends BaseObject  implements Persistent {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setAbeReqUid($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setAbeUid($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setAppUid($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setDelIndex($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setAbeReqSentTo($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setAbeReqSubject($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setAbeReqBody($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setAbeReqDate($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setAbeReqStatus($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setAbeReqAnswered($arr[$keys[9]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_UID)) $criteria->add(AbeRequestsPeer::ABE_REQ_UID, $this->abe_req_uid);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_UID)) $criteria->add(AbeRequestsPeer::ABE_UID, $this->abe_uid);
		if ($this->isColumnModified(AbeRequestsPeer::APP_UID)) $criteria->add(AbeRequestsPeer::APP_UID, $this->app_uid);
		if ($this->isColumnModified(AbeRequestsPeer::DEL_INDEX)) $criteria->add(AbeRequestsPeer::DEL_INDEX, $this->del_index);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_SENT_TO)) $criteria->add(AbeRequestsPeer::ABE_REQ_SENT_TO, $this->abe_req_sent_to);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_SUBJECT)) $criteria->add(AbeRequestsPeer::ABE_REQ_SUBJECT, $this->abe_req_subject);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_BODY)) $criteria->add(AbeRequestsPeer::ABE_REQ_BODY, $this->abe_req_body);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_DATE)) $criteria->add(AbeRequestsPeer::ABE_REQ_DATE, $this->abe_req_date);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_STATUS)) $criteria->add(AbeRequestsPeer::ABE_REQ_STATUS, $this->abe_req_status);
		if ($this->isColumnModified(AbeRequestsPeer::ABE_REQ_ANSWERED)) $criteria->add(AbeRequestsPeer::ABE_REQ_ANSWERED, $this->abe_req_answered);
