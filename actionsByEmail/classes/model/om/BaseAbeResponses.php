...
abstract class BaseAbeResponses extends BaseObject  implements Persistent {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setAbeResUid($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setAbeReqUid($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setAbeResClientIp($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setAbeResData($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setAbeResDate($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setAbeResStatus($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setAbeResMessage($arr[$keys[6]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AbeResponsesPeer::ABE_RES_UID)) $criteria->add(AbeResponsesPeer::ABE_RES_UID, $this->abe_res_uid);
		if ($this->isColumnModified(AbeResponsesPeer::ABE_REQ_UID)) $criteria->add(AbeResponsesPeer::ABE_REQ_UID, $this->abe_req_uid);
		if ($this->isColumnModified(AbeResponsesPeer::ABE_RES_CLIENT_IP)) $criteria->add(AbeResponsesPeer::ABE_RES_CLIENT_IP, $this->abe_res_client_ip);
		if ($this->isColumnModified(AbeResponsesPeer::ABE_RES_DATA)) $criteria->add(AbeResponsesPeer::ABE_RES_DATA, $this->abe_res_data);
		if ($this->isColumnModified(AbeResponsesPeer::ABE_RES_DATE)) $criteria->add(AbeResponsesPeer::ABE_RES_DATE, $this->abe_res_date);
		if ($this->isColumnModified(AbeResponsesPeer::ABE_RES_STATUS)) $criteria->add(AbeResponsesPeer::ABE_RES_STATUS, $this->abe_res_status);
		if ($this->isColumnModified(AbeResponsesPeer::ABE_RES_MESSAGE)) $criteria->add(AbeResponsesPeer::ABE_RES_MESSAGE, $this->abe_res_message);
